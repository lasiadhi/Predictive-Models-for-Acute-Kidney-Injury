...
data_pred <- read.csv("Results/AKI_Overall/IntraOp+preOp_probs_only/akiOverall_prediction_from_intraOP+preOP_probs_from_train2039.csv",header = TRUE)
...
ggplot(plot_data1,aes(map,time,z=risk)) + geom_tile(aes(fill=risk)) + 
  ...
  labs(x="Mean Arterial Blood pressure (mmHG)",y="Time (mins)",title="AKI-Overall Risk over the MAP and time" ) + 
