...
data_temp <- as.data.frame(cbind(data_all$map_Min, data_all$hr_Max, data_all$mac_range_0_1.1, data_all$aki_overall))  # change here for the outcome
...
svg('MAP_MAC_HR_AKIOverall_risk_for 2913patients_with_trueOutcome_cutoff_Youden.svg')
scatterplot3d(x = model1$model$MAP_min, y = model1$model$HR_max, z=model1$model$MAC_0_1.1,
              ...
              zlab="Time MAC less than 1.1 (mins)",angle=20, main="Predicted Risk for AKI-Overall", xlim = c(0,120), zlim=c(0,1800),box=FALSE, pch = 19)
dev.off()
