...
Model: IntraOp + PreOp prediction score: (AKI_Overall)
...
def fitRandomForestClassifier(X_train, X_test, y_train, y_test):
    ...
    clf = RandomForestClassifier(n_jobs=-1, random_state=0, warm_start=True)   
...
#    n_est               = [2000]
#    min_sample_leaf     = [1,2,5,8]
#    kbest_vals          = [70, 80, 90 ]
#    max_features        = [30, 35, 40, 50]   # (0, n_features)
    
    n_est               = [2000]
    min_sample_leaf     = [8]
    kbest_vals          = [80]
    max_features        = [30]   # (0, n_features)
...
#    ########################## End of upsampling ##########################################
    ...
    FILENAME_preds_from_preOp = "PreOp_model/Results/AKI_overall/esrd_dropped_final_features_model/akiOverall_PreOp_trainBy2038_predScore_for_wholeCohort_esrd_Dropped.csv"
    df_preOp_preds = pd.read_csv(FILENAME_preds_from_preOp, usecols=['acc','aki_predScore'], index_col=1)
    df_preOp_preds.columns = ['preOp_akiOverall_preds']
    ...
    # testing: use stratfied preOp results as a feature
    #X_train.preOp_akiOverall_preds = (X_train.preOp_akiOverall_preds>0.5)*1
    #X_test.preOp_akiOverall_preds = (X_test.preOp_akiOverall_preds>0.5)*1
...
#    plt.plot(fpr, tpr, lw=1, color='green', label='Logistic GAM (%0.2f)'% roc_auc)
    ...
    #plt.figure()
    #plot_learning_curves(X_train, y_train, X_test, y_test, CV_rfc, print_model=False, style='ggplot')
    #plt.show()
    ...
    #df_results.to_csv("akiOverall_prediction_from_intraOP+preOP_probs_from_test873.csv",index=False)
