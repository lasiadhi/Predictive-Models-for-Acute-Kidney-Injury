...
Return the model AUC and prediction scores - AKI Overall
"""
...
#    n_est               = [1200, 1300, 1400]
#    min_sample_leaf     = [7, 9, 11]
#    kbest_vals          = [190, 200, 210, 'all']
#    max_features        = [20, 25, 32]
...
if __name__ == '__main__':
    ...
    #akiOverall_y_test = y_test.reset_index(level=0)
    #akiOverall_y_test.to_csv("akiOverall_y_test_new.csv",index=False)
    #akiOverall_y_train = y_train.reset_index(level=0)
    #akiOverall_y_train.to_csv("akiOverall_y_train_new.csv",index=False) 
    
    #akiOverall_X_test = X_test.reset_index(level=0)
    #akiOverall_X_test.to_csv("akiOverall_X_test.csv",index=False)
    #akiOverall_X_train = X_train.reset_index(level=0)
    #akiOverall_X_train.to_csv("akiOverall_X_train.csv",index=False)
...
#    plt.plot(fpr, tpr, lw=1, color='green', label='Logistic GAM (%0.2f)'% roc_auc)
    ...
    df_results.to_csv("akiOverall_prediction_from_intraOP.csv",index=False)
