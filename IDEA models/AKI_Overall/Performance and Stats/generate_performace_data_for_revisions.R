...
# load train dataset: true outcome and predictions
data_results <- read.csv("Model/Results/AKI_Overall/IntraOp+PreOp_fullData_drop_esrd/akiallDay_full_model_Fulldata_prediction_from_train2038.csv",header = TRUE) #testing data or train data

################

metric_kdigo_corr <-  calculate_metric(data_results$true_AKI_outcome, data_results$pred_from_full_model)  #CHANHE HERE # generate PPV, ACC, Youden's_index, etc
...
write.csv(metric_kdigo_corr,"akiAllDay_PostOp_Full_model_ROC_AUC_ACC_PPV_NPV_F1_trainData.csv")

######### Plot ROC curve ############
roc_obj <- roc(data_results$true_AKI_outcome, data_results$pred_from_intraOp)
...
############################3 for testing data ###############################
data_results <- read.csv("Model/Results/AKI_Overall/IntraOp+PreOp_fullData_drop_esrd/akiOverall_intraOP+preOPFulldata_prediction_from_test873.csv",header = TRUE) #testing data

################

metric_kdigo_corr <-  calculate_metric(data_results$true_AKI_outcome, data_results$pred_from_intraPreOp)  #CHANHE HERE # generate PPV, ACC, Youden's_index, etc
#cutoff1_kdigo_corr <- metric_kdigo_corr[which.max(metric_kdigo_corr[,7]),1]   # cutoff1_kdigo_corr = the threshold where the Youden's index is maximum
#cutoff2_kdigo_corr <- calculate_cutoff2(metric_kdigo_corr[,1:2]) # get cutoff2 by checking the plateau of the accuracy curve
# data_new_kdigo_corr$category <- ifelse(data_new_kdigo_corr$kdigo_corrpred<=cutoff1_kdigo_corr,"low",
#                                        ifelse(data_new_kdigo_corr$kdigo_corrpred>cutoff2_kdigo_corr,"high","moderate"))
...
#write.csv(cbind(data_test$acc,data_new_kdigo_corr[,c('kdigo_corrpred','outcome','category')]),"aki3Day_prediction_result.csv")
write.csv(metric_kdigo_corr,"akiAllDay_post_full_model_ROC_AUC_ACC_PPV_NPV_F1_TestData.csv")
