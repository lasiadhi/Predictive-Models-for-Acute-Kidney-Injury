...
#    max_features        = ['auto', 'log2', 25, 30, 35, 40]
    ...
    n_est               = [1400, 1500, 1600]
    min_sample_leaf     = [6,7,8,11]
    kbest_vals          = [190, 200, 215, 'all']
    max_features        = [30, 32, 50]
...
if __name__ == '__main__':
    ...
    #aki7Day_y_test = y_test.reset_index(level=0)
    #aki7Day_y_test.to_csv("aki7Day_y_test_new.csv",index=False)
    #aki7Day_y_train = y_train.reset_index(level=0)
    #aki7Day_y_train.to_csv("aki7Day_y_train_new.csv",index=False) 
    
    #aki7Day_X_test = X_test.reset_index(level=0)
    #aki7Day_X_test.to_csv("aki7Day_X_test.csv",index=False)
    #aki7Day_X_train = X_train.reset_index(level=0)
    #aki7Day_X_train.to_csv("aki7Day_X_train.csv",index=False)
...
#    plt.plot(fpr, tpr, lw=1, color='green', label='Logistic GAM (%0.2f)'% roc_auc)
    ...
    #df_results.to_csv("aki7Day_prediction_from_intraOP.csv",index=False)
