...
Model: IntraOp + PreOp prediction score 
...
#    min_sample_leaf     = [1, 8, 9, 10]
#    kbest_vals          = [80, 100, 150, 180, 200, 'all']
#    max_features        = ['auto', 'log2', 15, 20, 25, 30]
    
#    n_est               = [1700, 2000]
#    min_sample_leaf     = [9, 11,15]
#    kbest_vals          = [60, 90, 100, 110]
...
#    max_features        = [40]   # (0, n_features)
    ...
    ##Report time only:
    n_est               = [1000, 1300, 1500, 1700, 2000]
    min_sample_leaf     = [1, 8, 9, 10, 12]
    kbest_vals          = [80, 110, 150, 180, 200, 'all']
    max_features        = ['auto', 'log2', 15, 20, 25, 40, 50]
...
#    ########################## End of upsampling ##########################################
    ...
    #dff.to_csv('important_features_f_value_aki7_intra+preOPpred.csv',index=False)
...
#    plt.plot(fpr, tpr, lw=1, color='green', label='Logistic GAM (%0.2f)'% roc_auc)
    ...
    #from mlxtend.plotting import plot_learning_curves
#    plt.figure()
#    plot_learning_curves(X_train, y_train, X_test, y_test, CV_rfc, print_model=False, style='ggplot', scoring='roc_auc')
#    plt.show()
...
#        print("%d. feature: %s (%f)" % (f + 1, X_important_train.columns[indices[f]], importances[indices[f]]))
    ...
    #df_results.to_csv("aki7Day_prediction_from_intraOP+preOP_probs_from_test874.csv",index=False)
