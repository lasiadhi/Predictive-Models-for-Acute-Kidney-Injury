...
#    max_features        = [50]
    ...
    min_sample_leaf     = [1, 8, 9, 10, 12, 15]
    kbest_vals          = [80, 110, 150, 180, 200, 230, 240, 250, 'all']
    max_features        = ['auto', 'log2', 15, 20, 25, 40, 50, 60, 70]
...
#    plt.plot(fpr, tpr, lw=1, color='green', label='Logistic GAM (%0.2f)'% roc_auc)
    ...
    for f in range(len(importances)):
        print("%d. feature: %s (%f)" % (f + 1, iXtest.columns[indices[f]], importances[indices[f]]))
    ...
    #df_results.to_csv("aki7Day_intraOP+preOPFulldata_prediction_from_test873.csv",index=False)
