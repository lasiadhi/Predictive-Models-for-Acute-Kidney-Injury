...
data_results <- read.csv("Model/Results/AKI_7Day/IntraOp+PreOp_probs_drop_esrd/aki7Day_prediction_from_intraOP+preOP_probs_from_train2038.csv",header = TRUE) #testing data or train data
...
write.csv(metric_kdigo_corr,"aki7Day_IntraOp+preOP_ROC_AUC_ACC_PPV_NPV_F1_trainData.csv")
...
############################3 for testing data ###############################
data_results <- read.csv("Model/Results/AKI_7Day/IntraOp+PreOp_probs_drop_esrd/aki7Day_prediction_from_intraOP+preOP_probs_from_test874.csv",header = TRUE) #testing data
...
#write.csv(cbind(data_test$acc,data_new_kdigo_corr[,c('kdigo_corrpred','outcome','category')]),"aki3Day_prediction_result.csv")
write.csv(metric_kdigo_corr,"aki7Day_IntraOp+preOP_ROC_AUC_ACC_PPV_NPV_F1_TestData.csv")
