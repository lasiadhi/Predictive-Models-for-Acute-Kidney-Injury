...
data_temp <- as.data.frame(cbind(data_all$map_Min, data_all$hr_Max, data_all$mac_range_0_1, data_all$aki3day))  # change here for the outcome
colnames(data_temp) <- c("MAP_min","HR_max","MAC_0_1","outcome")

s1 <- as.formula("outcome~s(MAP_min,k=5)+s(MAC_0_1,k=5)+s(HR_max,k=5)")
...
scatterplot3d(x = model1$model$MAP_min, y = model1$model$HR_max, z=model1$model$MAC_0_1,
              color = c("green","red")[risk],xlab="Minimum MAP (mmHG)",ylab="Maximum Heart Rate (bpm)",
              zlab="Time MAC less than 1.0 (mins)",angle=20, main="Fitted risk for AKI-3Day from simple GAM model", xlim = c(0,120), zlim=c(0,1800),box=FALSE, pch = 19)
#dev.off()
