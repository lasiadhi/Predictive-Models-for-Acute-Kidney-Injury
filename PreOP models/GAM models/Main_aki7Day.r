...
#setwd("S:/2016_223 IDEALIST/ANALYTIC CORE/MySurgeryRisk PostOP V1.0/3 Users/Lasith/IntraOp_model-DECLARE/PreOp_model")
setwd("/run/user/2209058/gvfs/smb-share:server=ahcdfs.ahc.ufl.edu,share=files/dom/SHARE/2016_223 IDEALIST/ANALYTIC CORE/MySurgeryRisk PostOP V1.0/3 Users/Lasith/IntraOp_model-DECLARE/PreOp_model")
...
data_train <- read.csv("Data_to_train/TrainData_with_aki7_2038samples_drop_esrd_patient.csv",header = TRUE) # training data
data_test <- read.csv("Data_to_test/TestData_with_aki7_drop_esrd_patient.csv",header = TRUE) #testing data
...
features_all <- read.csv("Data/feature_list_12_7_2017.csv",header = TRUE)   # considered final list from Amir - Ugh!
...
start_time <- Sys.time()
proc_data <- gen_proc_data(data_raw,features_selected,data_raw$aki7day) #generate data for modelling
...
#write.csv(metric_kdigo_corr,"aki7Day_PreOp_trainBy2038_ROC_AUC_ACC_PPV_NPV_F1_trainData_esrd_dropped.csv")
...
write.csv(cbind(data_test$acc,data_new_kdigo_corr[,c('kdigo_corrpred','outcome')]),"aki7Day_preOp_trainBy2038_prediction_result_on873Test_esrd_dropped.csv")
write.csv(metric_kdigo_corr_test,"aki7Day_PreOp_trainBy2038_ROC_AUC_ACC_PPV_NPV_F1_873testData_esrd_dropped.csv")
...
roc_obj <- roc(data_new_kdigo_corr$outcome, data_new_kdigo_corr$kdigo_corrpred)
svg('AKI7Day_PreOp_trainBy2038_ROC_with_873testData_esrd_dropped.svg')
...
write.csv(data_all,"aki7Day_PreOp_trainBy2038_predScore_for_wholeCohort_esrd_Dropped.csv")  # write the predition on the whole cohort to input as a feature to the intraOp model
