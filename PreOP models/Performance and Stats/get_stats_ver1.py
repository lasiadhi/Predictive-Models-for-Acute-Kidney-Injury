...
if __name__ == '__main__':
    ...
    FILENAME_X_train = "PreOp_model/Data_to_train/TrainData_with_akiOverall_2038samples_drop_esrd_patient.csv"
    FILENAME_X_test = "PreOp_model/Data_to_test/TestData_with_akiOverall_drop_esrd_patient.csv"

    Xtrain_df = pd.read_csv(FILENAME_X_train, index_col=0)
    Xtest_df = pd.read_csv(FILENAME_X_test, index_col=0)

    
    # merge all
    df_features = pd.concat([Xtrain_df, Xtest_df])
    ...
    my_dic = get_dic(df_var, 'variable', 'original_variable_type')  # generate python dictionary based on our feature dictionary. Change the column headers as necessary. 
    ...
    #df.to_csv('preOp_preProcessed_stats_for_aki_overall_continous_vars_MEDIAN.csv', index=True)
    df.to_csv('preOp_preProcessed_stats_for_aki_overall_continous_vars_MEAN_SD.csv', index=True)
    
    ###################### compute descriptives for categorical variables ##########################
    
    df_cat = generate_stats_for_cat_NEW(df_features, my_dic, outcome='aki_overall')  # compute stats for each outcome category
    
    # uncomment the following line to write stats for cat variables to the disk
    df_cat.to_csv('preOp_preProcessed_stats_for_aki_overall_CAT_vars.csv', index=False)  
