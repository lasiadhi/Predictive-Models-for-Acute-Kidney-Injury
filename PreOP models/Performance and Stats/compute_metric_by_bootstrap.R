# Lasith Adhikari
# This script is used to compute AUC with 95% CI for all the models
...
preds <- read.csv("PreOp_model/Results/AKI_overall/esrd_dropped_final_features_model/akiOverall_preOp_trainBy2038_prediction_result_on873Test_esrd_dropped.csv",header = TRUE) # data

optimal.cutpoint.Youden<-optimal.cutpoints(X = "kdigo_corrpred", status = "outcome", tag.healthy = 0, 
...
#set path to prediction scores and true values of testing cohort
preds <- read.csv("Model/Results/AKI_Overall/IntraOp+PreOp_probs_drop_esrd/akiOverall_prediction_from_intraOP+preOP_probs_from_test873.csv",header = TRUE) # data


df <- data.frame(preds$true_AKI_outcome, preds$pred_from_intraOp)
threshold = 0.44
