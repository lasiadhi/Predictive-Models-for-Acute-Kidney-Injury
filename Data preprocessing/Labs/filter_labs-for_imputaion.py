...
In this file, we first extract important labs and label them as intraOp 'yes' or not depending on the collect_date only (we check this datetime with the first surgery time stamps). Then we extract features (min, max, mean, var, count) for 
important 22 lab tests 

Also, we are adding abrnormal lab features in this update. 

# LAB FEATURES ON SURGERY DAY
"""
...
def filter_labs():
    ...
    preOp_df = pd.read_csv(FILENAME_preOp, sep=',', iterator=True, chunksize=20000, usecols=mycols) 
    for df in preOp_df:
...
def isIntraOp_lab():
    ...
    for df in labs_df:
        ...
        df['interOp_y_n'] = df.apply(lambda row: 'yes' if ((str(row['acc']) in dic) and ((pd.to_datetime(row['coll_datetime']) <= min(dic[str(row['acc'])])[1]+timedelta(days=1)) and (pd.to_datetime(row['coll_datetime']) >= min(dic[str(row['acc'])])[0]))) else 'no', axis=1)
...
def extract_lab_features():
    ...
    ################### compute abnormal percentatges for each lab based on the loinc normal ranges ######################################
    # COHBA:
...
if __name__ == '__main__':
    ...
    #for ubuntu:
    #FILENAME_filteredLabs = "/run/user/2209058/gvfs/smb-share:server=ahcdfs.ahc.ufl.edu,share=files/dom/SHARE/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/selected_labs.csv"
    #FILENAME_preOp = "/run/user/2209058/gvfs/smb-share:server=ahcdfs.ahc.ufl.edu,share=files/dom/SHARE/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/old_data/preop_labs.csv"
    ...
    #FILENAME_filterLabsWithYN = "/run/user/2209058/gvfs/smb-share:server=ahcdfs.ahc.ufl.edu,share=files/dom/SHARE/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/selected_labs_surgeryDayYN.csv"
    #FILENAME_intraOpLabs = '/run/user/2209058/gvfs/smb-share:server=ahcdfs.ahc.ufl.edu,share=files/dom/SHARE/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/intraOp_DAY_labs.csv'
    #FILENAME_labFeatures = '/run/user/2209058/gvfs/smb-share:server=ahcdfs.ahc.ufl.edu,share=files/dom/SHARE/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/lab_features_on_surgDay.csv'
    ...
    FILENAME_filterLabsWithYN = "S:/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/selected_labs_surgeryDayYN.csv"
    FILENAME_intraOpLabs = 'S:/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/intraOp_DAY_labs.csv'
    FILENAME_labFeatures = "S:/2016_223 IDEALIST/ANALYTIC CORE/2 MySurgeryRisk PostOP V1.0/3 Users/Lasith/Labs/cleaned_data/lab_features_on_surgDay.csv"
