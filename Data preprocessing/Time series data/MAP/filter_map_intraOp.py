...
This code filter out MAP values those are out side of the surgery time
...
def isIntraOp_lab():
    ...
    for df in TS_df:
        df['interOp_y_n'] = df.apply(lambda row: 'yes' if ((str(row['Acc_No']) in dic) and (pd.to_datetime(row['time_stamp']) <= min(dic[str(row['Acc_No'])])[1]) and (pd.to_datetime(row['time_stamp']) >= min(dic[str(row['Acc_No'])])[0])) else 'no', axis=1)
        df.to_csv(FILENAME_selected_TSYN, mode='a',header=False, index=False)
    print ('Done isIntraOp!')
    
    
def readIntraOp_TS():
    selected_TS_df = pd.read_csv(FILENAME_selected_TSYN, names=['no','time_stamp','systolic',	'diastolic',	'map','Acc_No','interOp_y_n'], index_col=False, iterator=True, chunksize=20000)
    writeHeader= True
...
if __name__ == '__main__':
    ...
    FILENAME_TS = "Time_series/Clean_data/bp_invasive_clean.csv"

    #Write
    FILENAME_selected_TS = "MAP_onlyIntraOp.csv"
    FILENAME_selected_TSYN = "MAP_interOpYN.csv"
