...
fun_extract_par <- function(var,bp_data_cleaned,lower_limits,upper_limits,trun_check,interval_list){
  ...
  for(i in 1:length(unique_accs)){
    ...
    check_gap <- truncate_fun(temp_data$time_stamp,trun_check)
    if(check_gap$check){
    ...
    if(nrow(temp_data)<15){
      ...
      issue_accs$acc[iss_ind] <- temp_data$acc[1] ; issue_accs$start_time[iss_ind] <- as.character(temp_data$time_stamp[1])
      issue_accs$end_time[iss_ind] <- as.character(temp_data$time_stamp[nrow(temp_data)]) ; iss_ind <- iss_ind+1
      next
    }
    
    if(i==1){
      result <- para_extract(temp_data$obser,lower_limits,upper_limits,interval_list) 
      result$acc <- unique_accs[i] ; next
    }else{
      temp_result <- para_extract(temp_data$obser,lower_limits,upper_limits,interval_list) 
