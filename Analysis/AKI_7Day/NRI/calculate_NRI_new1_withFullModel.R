# Lasith Adhikari
# Computing NRI for AKI-7Day: Postop full model Vs. PreOp model 
...
preds <- read.csv("Analysis/AKI_7Day/Data/categorized_results_preOp+intraPreOp.csv",header = TRUE) # data

cutoff_preOp = 0.43
#cutoff_intraPreOp = 0.41
cutoff_intraPreOp = 0.40    # full model cutoff
...
# for event:
ind <- which(preds$true_AKI7_outcome==1)
event <-  preds[ind,]
...
# for non event:
ind <- which(preds$true_AKI7_outcome==0)
nonevent <-  preds[ind,]
...
NRI <- function(preds, indices){
  ...
  # for event:
  ind <- which(preds_i$true_AKI7_outcome==1)
  event <-  preds_i[ind,]
  event_up <- length(which(event$preOp_cut==0 & event$intraPreOp_cut==1))  # up: 0 -> 1
  event_down <-  length(which(event$preOp_cut==1 & event$intraPreOp_cut==0))  # down: 1 -> 0
  event_0_0 <- length(which(event$preOp_cut==0 & event$intraPreOp_cut==0))
  ...
  # for non event:
  ind <- which(preds_i$true_AKI7_outcome==0)
