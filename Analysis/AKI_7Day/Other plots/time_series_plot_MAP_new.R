...
# read data
time_series_data <- read.csv("Time_series/Clean_data/IntraOp_clean_data/MAP_onlyIntraOp.csv",header = TRUE) # data
# filter and preprocess data
time_series_data$time_stamp <- as.POSIXct(time_series_data$time_stamp, format="%Y-%m-%d %H:%M:%S")
time_series_data$map <- as.numeric(as.character(time_series_data$map))
...
make_plot <- function(time_in_mins, data, myflag, mycolor){
  ...
  if (myflag){
    matplot(time_in_mins, data, type="l", lty=1, col=mycolor, xlab="Time (mins)", ylab="MAP (mmHg)", xlim = c(0, max_time), ylim = c(0,max_obser))
  }
  matlines(time_in_mins, data, type="l", lty=1, col=mycolor, xlab="Time (mins)", ylab="MAP (mmHg)", xlim = c(0, max_time), ylim = c(0,max_obser))
}
...
for (acc_i in accts_noAKI$acc){
  time_series_data_i <- time_series_data[which(time_series_data$Acc_No == acc_i),]
  ...
  if(dim(time_series_data_i)[1] > 400){
    ...
    make_plot(time_series_data_i$time_stamp, time_series_data_i$map, myflag, mycolor)
...
df_noAKI$map <- as.numeric(as.character(df_noAKI$map))
#x <- CI(as.numeric(as.character(df_noAKI$obser)), ci=0.95)
mean_data_noAKI <- group_by(df_noAKI, time_stamp)%>% summarise(mean = mean(map, na.rm = TRUE), sd = sd(map))
mytime <- seq(0,max_time, 0.01)
matlines(mean_data_noAKI$time_stamp, mean_data_noAKI$mean, type="l", lty=1, lwd=0.4, col=rgb(0,1,0,1), xlim = c(0, max_time), ylim = c(0,max_obser))
...
df_AKI$map <- as.numeric(as.character(df_AKI$map))
mean_data_AKI <- group_by(df_AKI, time_stamp)%>% summarise(mean = mean(map, na.rm = TRUE), sd = sd(map))
matlines(mean_data_AKI$time_stamp, mean_data_AKI$mean, type="l", lty=1, lwd=0.4, col=rgb(1,0,0,1), xlim = c(0, max_time), ylim = c(0,max_obser))
...
legend("topright", c('Mean MAP for AKI','Mean MAP for No AKI', '95% CI for MAP (AKI)', '95% CI for MAP (No AKI)'), lty=c(1,1,2,2), lwd=c(1.5,1.5,1.5,1.5), col=c('red', rgb(0,1,0,1), rgb(1,0,0,1), rgb(0.156, 0.443, 0.243,1)))
...
for (i in 1:20){
  ...
  par(mfrow=c(2,1))
  scatterplot3d(xyz, type="l", color= rgb(1,0,0,1), box=FALSE, main = 'NO AKI', xlim = c(40, 200), ylim = c(40, 200) , zlim = c(40, 200), angle = 90)
  scatterplot3d(xyz_AKI, type="l", color= rgb(1,0,0,1), box=FALSE, main = 'AKI', xlim = c(40, 200) , ylim = c(40, 200) , zlim = c(40, 200), angle = 90)
  print(i*10)
  Sys.sleep(4)
}
